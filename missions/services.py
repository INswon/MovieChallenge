...
class MissionService:
    ...
    #「3本の映画達成」「1日3本の映画視聴達成」のミッションバッジ付与処理
    @staticmethod
    def check_and_complete_mission(user, mission_title):
        title_candidates = ALTERNATIVE_TITLES.get(mission_title, [mission_title])
        mission = Mission.objects.filter(title__in=title_candidates).first()

        if not mission:
            print(f"ミッション '{mission_title}' が存在しませんでした")
            return False, "ミッションが見つかりませんでした"

        watched_movies_count = UserMovieRecord.objects.filter(
    ...
    def check_and_assign_genre_batch(user):
        title_candidates = ALTERNATIVE_TITLES.get("3ジャンル制覇", ["3ジャンル制覇"])
        mission = Mission.objects.filter(title__in=title_candidates).first()
        if not mission:
