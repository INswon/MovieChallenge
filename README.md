...
https://bmwmh8hnta.ap-northeast-1.awsapprunner.com 
...
## 4.補足
現在、コア機能を優先して段階的リリースを実施しています。

現在動作中の機能（本番環境で確認可能）:
ユーザー認証・セッション管理
映画記録のCRUD操作（作成・編集・削除・表示）
PostgreSQL連携とデータ永続化

調整中の機能:
TMDB API連携（外部API通信）
アプリケーションコード：実装完了・ローカル動作確認済み
本番環境：AWS App RunnerのVPCネットワーク設定調整中
